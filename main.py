...
def main():
    ...
    lh_train_input = np.concatenate([lh_classifications.to_numpy(), features_train], axis=1)
    rh_train_input = np.concatenate([rh_classifications.to_numpy(), features_train], axis=1)
    lh_val_input = np.concatenate([lh_classifications_val.to_numpy(), features_val], axis=1)
    rh_val_input = np.concatenate([rh_classifications_val.to_numpy(), features_val], axis=1)
    ...
    #rint("________ Visualize ________")

    lh_correlation, rh_correlation = predAccuracy(lh_fmri_val_pred, lh_fmri_val, rh_fmri_val_pred, rh_fmri_val)

    #visualize.plotFMRIfromIMGandROI(args, train_img_dir, train_img_list, lh_fmri_val_pred, rh_fmri_val_pred, 'EBA', 0,
    #                                'left')
    #print("real")
    #visualize.plotFMRIfromIMG(args, test_img_dir, test_img_list, lh_fmri, rh_fmri)
    #print("pred")
    #visualize.plotFMRIfromIMG(args, test_img_dir, test_img_list, lh_fmri_val_pred, rh_fmri_val_pred)
